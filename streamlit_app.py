...
for t in tasks:
    if not isinstance(t, dict):
        continue
    try:
        deadline_dt = datetime.fromisoformat(str(t.get("deadline"))).astimezone(timezone.utc)
    except:
        continue
    ...
    with col1:
        st.markdown(
            ...
            ⏰ 開始目安：{t.get('start_time','00:00')}（あと {remaining} 分）<br>
...
for t in tasks:
    ...
    try:
        ...
        if d in dates:
            calendar[d.strftime("%m/%d")].append(t.get("title","(無題)"))
    except:
        continue
...
for t in tasks:
    if not isinstance(t, dict):
        continue
    try:
        deadline_dt = datetime.fromisoformat(str(t.get("deadline")))
    except:
        continue
